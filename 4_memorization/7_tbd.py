...
def print_examples(dataset, threshold, n=5):
    ...
    for i, (_, row) in enumerate(examples.iterrows()):
        if i >= n:
            break
        print(f"Example {i+1}:")
        print(f"Q2: {row['Memorization q2']}")
        print(f"GPT-4 Answer: {row['Memorization Answer GPT4o']}")
...
plt.bar(combined_result["Dataset"], combined_result["Percentage of Near-Exact Matches"])
